...
void RegisterLuaFunctions_Math( lua_State* L )
{
    ...
    sol::usertype<vec2> vec2_type =
        lua.new_usertype<vec2>( "vec2", sol::constructors<vec2(), vec2( float ), vec2( const vec2& ), vec2( float, float )>() );
    vec2_type["x"] = &vec2::x;
    vec2_type["y"] = &vec2::y;
    ...
    sol::usertype<vec3> vec3_type =
        lua.new_usertype<vec3>( "vec3", sol::constructors<vec3(), vec3( float ), vec3( const vec3& ), vec3( float, float, float )>() );
    vec3_type["x"] = &vec3::x;
    vec3_type["y"] = &vec3::y;
    vec3_type["z"] = &vec3::z;
    ...
    sol::usertype<vec4> vec4_type = lua.new_usertype<vec4>(
        "vec4", sol::constructors<vec4(), vec4( float ), vec4( const vec4& ), vec4( float, float, float, float )>() );
    vec4_type["x"] = &vec4::x;
    vec4_type["y"] = &vec4::y;
    vec4_type["z"] = &vec4::z;
    vec4_type["w"] = &vec4::w;
