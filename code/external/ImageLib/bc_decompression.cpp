...
static void Color565To888( uint16_t color565, uint32_t& r, uint32_t& g, uint32_t& b )
{
    ...
    // the average reconstruction error is actually better with that method (depending on how the data was packed). It is just more
    // expensive than a couple bitshifts and adds
...
static void Decompress_BC1_Block( const uint8_t* compressedBlock, uint8_t decompressedBlock[64] )
{
    ...
    {
        // the +1 is for rounding when doing integer division / 3. Ex: 0 / 3 = 0, 1 / 3 = 0, 2 / 3 = 0, but we want the 2 / 3 to round up to
        // 1
        ...
        for ( int i = 0; i < 16; ++i )
        {
            ...
            switch ( index )
            {
            case 0: decompressed[i] = c0; break;
            case 1: decompressed[i] = c1; break;
            case 2: decompressed[i] = c2; break;
            case 3: decompressed[i] = c3; break;
            }
        }
    }
    else
...
static void BC6_ExtractEndpoints( uint32_t modeBits, const uint8_t* compressedBlock, int packedEndpoints[4][3] )
{
    ...
    case 0:
    {
        ...
        packedEndpoints[3][2] = ExtractBitSegment( compressedBlock, 50, 1 ) | ( ExtractBitSegment( compressedBlock, 60, 1 ) << 1 ) |
                                ( ExtractBitSegment( compressedBlock, 70, 1 ) << 2 ) |
                                ( ExtractBitSegment( compressedBlock, 76, 1 ) << 3 ) | ( ExtractBitSegment( compressedBlock, 4, 1 ) << 4 );
        break;
    }
    case 1:
    {
        ...
        packedEndpoints[3][2] = ExtractBitSegment( compressedBlock, 12, 2 ) | ( ExtractBitSegment( compressedBlock, 23, 1 ) << 2 ) |
                                ( ExtractBitSegment( compressedBlock, 32, 1 ) << 3 ) |
                                ( ExtractBitSegment( compressedBlock, 34, 1 ) << 4 ) | ( ExtractBitSegment( compressedBlock, 33, 1 ) << 5 );
        break;
    }
    case 2:
    {
    ...
    case 14:
    {
        ...
        packedEndpoints[3][2] = ExtractBitSegment( compressedBlock, 50, 1 ) | ( ExtractBitSegment( compressedBlock, 60, 1 ) << 1 ) |
                                ( ExtractBitSegment( compressedBlock, 70, 1 ) << 2 ) |
                                ( ExtractBitSegment( compressedBlock, 76, 1 ) << 3 ) | ( ExtractBitSegment( compressedBlock, 34, 1 ) << 4 );
        break;
    }
    case 18:
    {
    ...
    case 22:
    {
        ...
        packedEndpoints[2][0] = ExtractBitSegment( compressedBlock, 65, 5 );
        packedEndpoints[2][1] = ExtractBitSegment( compressedBlock, 41, 4 ) | ( ExtractBitSegment( compressedBlock, 24, 1 ) << 4 ) |
                                ( ExtractBitSegment( compressedBlock, 23, 1 ) << 5 );
        packedEndpoints[2][2] = ExtractBitSegment( compressedBlock, 61, 4 ) | ( ExtractBitSegment( compressedBlock, 14, 1 ) << 4 );
        packedEndpoints[3][0] = ExtractBitSegment( compressedBlock, 71, 5 );
        packedEndpoints[3][1] = ExtractBitSegment( compressedBlock, 51, 4 ) | ( ExtractBitSegment( compressedBlock, 40, 1 ) << 4 ) |
                                ( ExtractBitSegment( compressedBlock, 33, 1 ) << 5 );
    ...
    case 26:
    {
        ...
        packedEndpoints[3][0] = ExtractBitSegment( compressedBlock, 71, 5 );
        packedEndpoints[3][1] = ExtractBitSegment( compressedBlock, 51, 4 ) | ( ExtractBitSegment( compressedBlock, 40, 1 ) << 4 );
        packedEndpoints[3][2] = ExtractBitSegment( compressedBlock, 50, 1 ) | ( ExtractBitSegment( compressedBlock, 13, 1 ) << 1 ) |
                                ( ExtractBitSegment( compressedBlock, 70, 1 ) << 2 ) |
                                ( ExtractBitSegment( compressedBlock, 76, 1 ) << 3 ) |
                                ( ExtractBitSegment( compressedBlock, 34, 1 ) << 4 ) | ( ExtractBitSegment( compressedBlock, 33, 1 ) << 5 );
        break;
    }
    case 30:
    {
        ...
        packedEndpoints[2][0] = ExtractBitSegment( compressedBlock, 65, 6 );
        packedEndpoints[2][1] = ExtractBitSegment( compressedBlock, 41, 4 ) | ( ExtractBitSegment( compressedBlock, 24, 1 ) << 4 ) |
                                ( ExtractBitSegment( compressedBlock, 21, 1 ) << 5 );
        packedEndpoints[2][2] = ExtractBitSegment( compressedBlock, 61, 4 ) | ( ExtractBitSegment( compressedBlock, 14, 1 ) << 4 ) |
                                ( ExtractBitSegment( compressedBlock, 22, 1 ) << 5 );
        packedEndpoints[3][0] = ExtractBitSegment( compressedBlock, 71, 6 );
        packedEndpoints[3][1] = ExtractBitSegment( compressedBlock, 51, 4 ) | ( ExtractBitSegment( compressedBlock, 11, 1 ) << 4 ) |
                                ( ExtractBitSegment( compressedBlock, 31, 1 ) << 5 );
        packedEndpoints[3][2] = ExtractBitSegment( compressedBlock, 12, 2 ) | ( ExtractBitSegment( compressedBlock, 23, 1 ) << 2 ) |
                                ( ExtractBitSegment( compressedBlock, 32, 2 ) << 3 ) |
                                ( ExtractBitSegment( compressedBlock, 34, 1 ) << 4 ) | ( ExtractBitSegment( compressedBlock, 33, 1 ) << 5 );
        break;
    }
...
static void BC6_GeneratePaletteUnquantized(
    const BC6ModeInfo& info, uint8_t regionIdx, bool isSignedF16, const int endpoints[4][3], uint16_t palette[16][3] )
{
    ...
    for ( int chan = 0; chan < 3; ++chan )
    {
        ...
        for ( uint8_t idx = 0; idx < uNumIndices; ++idx )
        {
            ...
            palette[baseIdx + idx][chan] =
                BC6_FinishUnquantize( ( a * ( 64 - aWeights[idx] ) + b * aWeights[idx] + 32 ) >> 6, isSignedF16 );
        }
    }
}
...
static void BC7_FullyDecodeEndpoints(
    uint32_t mode, uint8_t endPoints[3][2][4], const uint8_t* compressedBlock, uint32_t& currentBitOffset )
{
    const BC7ModeBitCounts& info = s_bc7BitTable[mode];
...
static uint32_t BC7_Interpolate( uint32_t e0, uint32_t e1, uint8_t bitsPerIndex, int index )
{
    uint32_t lerpWeight;
    if ( bitsPerIndex == 2 )
        lerpWeight = indexInterpolationWeights_2Bit[index];
    else if ( bitsPerIndex == 3 )
        lerpWeight = indexInterpolationWeights_3Bit[index];
    else
        lerpWeight = indexInterpolationWeights_4Bit[index];
    ...
    for ( int blockY = 0; blockY < numBlocksY; ++blockY )
    {
        for ( int blockX = 0; blockX < numBlocksX; ++blockX )
        {
            ...
            if constexpr ( FORMAT == Underlying( ImageFormat::BC1_UNORM ) )
                Decompress_BC1_Block( compressedBlock, decompressedBlock );
            else if constexpr ( FORMAT == Underlying( ImageFormat::BC2_UNORM ) )
                Decompress_BC2_Block( compressedBlock, decompressedBlock );
            else if constexpr ( FORMAT == Underlying( ImageFormat::BC3_UNORM ) )
                Decompress_BC3_Block( compressedBlock, decompressedBlock );
            else if constexpr ( FORMAT == Underlying( ImageFormat::BC4_UNORM ) )
                Decompress_BC4_Block_UNorm( compressedBlock, decompressedBlock );
            else if constexpr ( FORMAT == Underlying( ImageFormat::BC4_SNORM ) )
                Decompress_BC4_Block_SNorm( (const int8_t*)compressedBlock, (int8_t*)decompressedBlock );
            else if constexpr ( FORMAT == Underlying( ImageFormat::BC5_UNORM ) )
                Decompress_BC5_Block_UNorm( compressedBlock, decompressedBlock );
            else if constexpr ( FORMAT == Underlying( ImageFormat::BC5_SNORM ) )
                Decompress_BC5_Block_SNorm( (const int8_t*)compressedBlock, (int8_t*)decompressedBlock );
            else if constexpr ( FORMAT == Underlying( ImageFormat::BC6H_U16F ) )
                Decompress_BC6_Block( compressedBlock, (uint16_t*)decompressedBlock, false );
            else if constexpr ( FORMAT == Underlying( ImageFormat::BC6H_S16F ) )
                Decompress_BC6_Block( compressedBlock, (uint16_t*)decompressedBlock, true );
            else if constexpr ( FORMAT == Underlying( ImageFormat::BC7_UNORM ) )
                Decompress_BC7_Block( compressedBlock, decompressedBlock );
...
RawImage2D DecompressBC( const RawImage2D& compressedImage )
{
    ...
    {
    case ImageFormat::BC1_UNORM: Decompress_BC_Internal<Underlying( ImageFormat::BC1_UNORM )>( compressedImage, decompressedImage ); break;
    case ImageFormat::BC2_UNORM: Decompress_BC_Internal<Underlying( ImageFormat::BC2_UNORM )>( compressedImage, decompressedImage ); break;
    case ImageFormat::BC3_UNORM: Decompress_BC_Internal<Underlying( ImageFormat::BC3_UNORM )>( compressedImage, decompressedImage ); break;
    case ImageFormat::BC4_UNORM: Decompress_BC_Internal<Underlying( ImageFormat::BC4_UNORM )>( compressedImage, decompressedImage ); break;
    case ImageFormat::BC4_SNORM: Decompress_BC_Internal<Underlying( ImageFormat::BC4_SNORM )>( compressedImage, decompressedImage ); break;
    case ImageFormat::BC5_UNORM: Decompress_BC_Internal<Underlying( ImageFormat::BC5_UNORM )>( compressedImage, decompressedImage ); break;
    case ImageFormat::BC5_SNORM: Decompress_BC_Internal<Underlying( ImageFormat::BC5_SNORM )>( compressedImage, decompressedImage ); break;
    case ImageFormat::BC6H_U16F: Decompress_BC_Internal<Underlying( ImageFormat::BC6H_U16F )>( compressedImage, decompressedImage ); break;
    case ImageFormat::BC6H_S16F: Decompress_BC_Internal<Underlying( ImageFormat::BC6H_S16F )>( compressedImage, decompressedImage ); break;
    case ImageFormat::BC7_UNORM: Decompress_BC_Internal<Underlying( ImageFormat::BC7_UNORM )>( compressedImage, decompressedImage ); break;
    }
