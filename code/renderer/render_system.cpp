...
static void InitPerFrameData()
{
    for ( int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i )
    {
        ...
        data.sceneConstantBuffer = rg.device.NewBuffer( sizeof( GpuData::SceneGlobals ), BUFFER_TYPE_UNIFORM,
            MEMORY_TYPE_HOST_VISIBLE | MEMORY_TYPE_HOST_COHERENT, "sceneConstantsUBO" + std::to_string( i ) );
        ...
        data.sceneConstantDescSet =
            rg.descriptorPool.NewDescriptorSet( litPipeline.GetResourceLayout()->sets[PG_SCENE_GLOBALS_DESCRIPTOR_SET] );
        bufferDescriptors   = { DescriptorBufferInfo( data.sceneConstantBuffer ) };
        writeDescriptorSets = {
            WriteDescriptorSet_Buffer(
                data.sceneConstantDescSet, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, PG_SCENE_CONSTS_BINDING_SLOT, &bufferDescriptors[0] ),
        };
        ...
        data.lightBuffer = rg.device.NewBuffer( PG_MAX_NUM_LIGHTS * sizeof( GpuData::PackedLight ), BUFFER_TYPE_STORAGE,
            MEMORY_TYPE_HOST_VISIBLE | MEMORY_TYPE_HOST_COHERENT, "lightSSBO" + std::to_string( i ) );
        ...
        writeDescriptorSets = {
            WriteDescriptorSet_Buffer( data.lightsDescSet, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 0, &bufferDescriptors[0] ),
        };
        rg.device.UpdateDescriptorSets( writeDescriptorSets );

        data.lightingAuxTexturesDescSet =
            rg.descriptorPool.NewDescriptorSet( litPipeline.GetResourceLayout()->sets[PG_LIGHTING_AUX_DESCRIPTOR_SET] );
        imgDescriptors = { DescriptorImageInfoNull(), DescriptorImageInfoNull(),
            DescriptorImageInfo( AssetManager::Get<GfxImage>( "brdfLUT" )->gpuTexture, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL ) };
        writeDescriptorSets =
            WriteDescriptorSet_Images( data.lightingAuxTexturesDescSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, imgDescriptors );
        rg.device.UpdateDescriptorSets( writeDescriptorSets );

        data.postProcessDescSet =
            rg.descriptorPool.NewDescriptorSet( postProcessPipeline.GetResourceLayout()->sets[PG_LIGHTING_AUX_DESCRIPTOR_SET] );
        imgDescriptors = {
            DescriptorImageInfo( s_renderGraph.GetPhysicalResource( "litOutput", i )->texture, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL ),
        };
        writeDescriptorSets =
            WriteDescriptorSet_Images( data.postProcessDescSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, imgDescriptors );
        rg.device.UpdateDescriptorSets( writeDescriptorSets );
    }
}
...
bool Init( uint32_t sceneWidth, uint32_t sceneHeight, bool headless )
{
    ...
    // BUFFERS + IMAGES
    {
        ...
        // clang-format on
        s_cubeVertexBuffer =
            rg.device.NewBuffer( sizeof( verts ), verts, BUFFER_TYPE_VERTEX, MEMORY_TYPE_DEVICE_LOCAL, "cube vertex buffer" );
        s_cubeIndexBuffer =
            rg.device.NewBuffer( sizeof( indices ), indices, BUFFER_TYPE_INDEX, MEMORY_TYPE_DEVICE_LOCAL, "cube index buffer" );
    }
...
void CreateTLAS( Scene* scene )
{
    ...
    Buffer instancesBuffer =
        rg.device.NewBuffer( bufferSize, BUFFER_TYPE_DEVICE_ADDRESS | BUFFER_TYPE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY,
            MEMORY_TYPE_HOST_VISIBLE | MEMORY_TYPE_HOST_COHERENT, "tlas" );
    ...
    scene->tlas = rg.device.NewAccelerationStructure(
        AccelerationStructureType::TLAS, accelerationStructureBuildSizesInfo.accelerationStructureSize );
...
PipelineAttachmentInfo GetPipelineAttachmentInfo( const std::string& name ) { return s_renderGraph.GetPipelineAttachmentInfo( name ); }
...
static void UpdateSkyboxAndBackground( Scene* scene )
{
    ...
    if ( scene->skybox )
    {
        if ( s_skyboxTextures[rg.currentFrame] != &scene->skybox->gpuTexture )
        {
            ...
            imgDescriptors = {
                DescriptorImageInfo( *s_skyboxTextures[rg.currentFrame], VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL ),
            ...
            writeDescriptorSets =
                WriteDescriptorSet_Images( frameData.skyboxDescSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, imgDescriptors );
            ...
            imgDescriptors      = { DescriptorImageInfo( scene->skyboxIrradiance->gpuTexture, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL ),
                     DescriptorImageInfo( scene->skyboxReflectionProbe->gpuTexture, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL ) };
            writeDescriptorSets = WriteDescriptorSet_Images(
                frameData.lightingAuxTexturesDescSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, imgDescriptors );
            rg.device.UpdateDescriptorSets( static_cast<uint32_t>( writeDescriptorSets.size() ), writeDescriptorSets.data() );
        }
    }
    else
    {
        if ( s_skyboxTextures[rg.currentFrame] )
        {
            ...
            rg.device.UpdateDescriptorSet(
                WriteDescriptorSet_Image( frameData.skyboxDescSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 0, &nullImg ) );
            rg.device.UpdateDescriptorSet(
                WriteDescriptorSet_Image( frameData.skyboxDescSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, &nullImg ) );
            rg.device.UpdateDescriptorSet(
                WriteDescriptorSet_Image( frameData.skyboxDescSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 2, &nullImg ) );

            rg.device.UpdateDescriptorSet(
                WriteDescriptorSet_Image( frameData.lightingAuxTexturesDescSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 0, &nullImg ) );
            rg.device.UpdateDescriptorSet(
                WriteDescriptorSet_Image( frameData.lightingAuxTexturesDescSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, &nullImg ) );
        }
        s_skyboxTextures[rg.currentFrame] = nullptr;
    }
}

static void UpdateDescriptors() { TextureManager::UpdateDescriptors( bindlessTexturesDescriptorSet ); }
...
void Render()
{
    ...
    RG_RenderData renderData{
        .scene = scene, .cmdBuf = &cmdBuf, .swapchain = &rg.swapchain, .swapchainImageIndex = rg.swapchainImageIndex };
    ...
    cmdBuf.EndRecording();
    rg.device.SubmitCommandBuffers(
        1, &cmdBuf, frameData.swapchainImgAvailableSemaphore, frameData.renderCompleteSemaphore, &frameData.inFlightFence );
