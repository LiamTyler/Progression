...
void NewDrawFunctionTG( ComputeTask* task, TGExecuteData* data )
{
    ...
    vkCmdBindDescriptorSets( cmdBuf, VK_PIPELINE_BIND_POINT_COMPUTE, s_gradientPipelineLayout, 0, 1, &TextureManager::GetBindlessSet(), 0, nullptr );
...
bool Init( uint32_t sceneWidth, uint32_t sceneHeight, uint32_t displayWidth, uint32_t displayHeight, bool headless )
{
    ...
    TGBTextureRef gradientImg = cTask->AddTextureOutput( "gradientImg", PixelFormat::R16_G16_B16_A16_FLOAT, vec4( 0, 1, 0, 1 ), SIZE_SCENE(), SIZE_SCENE() );
...
void OldDrawFunction( CommandBuffer& cmdBuf )
{
    ...
    ComputePushConstants push{ vec4( 1, 0, 0, 1 ), vec4( 0, 0, 1, 1 ), s_drawImg.GetBindlessArrayIndex() };
    vkCmdPushConstants( cmdBuf, s_gradientPipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof( ComputePushConstants ), &push );
